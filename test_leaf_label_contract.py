...
def test_label_rule_is_universal_and_states_its_consequence():
    contract = _leaf_label_contract(config())
    assert "Begin every worker goal" in contract
    ...
    assert "not optional" in contract
    assert "forces the leaf onto Sol" in contract


def test_label_rule_offers_only_tiers_the_operator_left_callable():
    contract = _leaf_label_contract(config(
        callable={"luna": True, "spark": False, "terra": True, "sol": True},
    ))
    assert "[spark]" not in contract
    assert "[luna]" in contract and "[sol]" in contract and "[terra]" in contract


def test_read_only_leaf_falls_back_to_luna_when_spark_is_off():
    cfg = config(callable={"luna": True, "spark": False, "terra": True, "sol": True})
    assert _read_only_leaf_tier(cfg) == "luna"
    clause = _read_only_delegation_clause(cfg)
    assert "[luna]" in clause and "model:luna" in clause
    assert "Spark" not in clause


def test_read_only_clause_is_dropped_when_no_read_only_tier_is_callable():
    cfg = config(callable={"luna": False, "spark": False, "terra": True, "sol": True})
    assert _read_only_leaf_tier(cfg) == ""
    assert _read_only_delegation_clause(cfg) == ""


def test_preflight_never_names_a_tier_the_conductor_cannot_spawn():
    cfg = config(callable={"luna": True, "spark": False, "terra": True, "sol": True})
    instruction, context = preflight_texts(cfg)
    for text in (instruction, context):
        assert "[spark]" not in text
        assert "model:spark" not in text
        assert "Begin every worker goal" in text


def test_conductor_contract_marker_survives_the_inserted_rule():
    ...
    assert "planning conductor." in context


def test_labelled_read_only_ui_leaf_no_longer_lands_on_sol():
    cfg = config()
    ...
    assert unlabelled.tier == "sol"
    assert labelled.tier == "luna"
    assert labelled.reason == "explicit [luna] override"
