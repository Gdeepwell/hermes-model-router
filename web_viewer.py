...
<style>
:root{color-scheme:dark;--bg:#080b12;--panel:#111723;--border:#253044;--text:#e9eef8;--muted:#8997ad;--luna:#61dafb;--spark:#f3f6fb;--terra:#88e36f;--sol:#ffb454;--opus5:#d695ff;--qwen:#38d9a9;--accent:#a78bfa}
...
async function loadSettings(){
  try{
    ...
    $('settings-status').textContent='Hiba: '+error.message;
...
function renderSettings(){
  ...
  const models=['luna','spark','terra','sol','opus5','qwen'];
  const modelLabels={luna:'GPT-5.6 Luna',spark:'GPT-5.3 Spark',terra:'GPT-5.6 Terra',sol:'GPT-5.6 Sol',opus5:'Claude Opus 5',qwen:'Qwen 3.7 Plus'};
  const modelDescriptions={luna:'Gyors, egyszerű feladatok',spark:'Read-only delegált munkák',terra:'Általános orchestrator',sol:'Biztonságkritikus, design',opus5:'Külső Claude Code bridge',qwen:'Alternatív modell'};
  for(const model of models){
    const enabled=callable[model]!==false;
    const item=document.createElement('div');
    item.className='toggle-item'+(enabled?'':' disabled');
    item.innerHTML=`
...
function initColumnResize(){const table=$('log-table'),cols=[...table.querySelectorAll('col')],heads=[...table.querySelectorAll('th')];let saved=null;try{saved=JSON.parse(localStorage.getItem(widthStore))}catch(e){}
 ...
 heads.forEach((head,i)=>{const grip=document.createElement('div');grip.className='resizer';grip.title='Húzd az oszlop szélességének módosításához · dupla kattintás: alaphelyzet';head.append(grip);
...
function detailsTable(group){const table=document.createElement('table');table.className='details-table';const colgroup=document.createElement('colgroup');for(const width of ['110px','90px','34%','80px','150px','auto']){const col=document.createElement('col');col.style.width=width;colgroup.append(col)}table.append(colgroup);const head=document.createElement('thead'),headRow=document.createElement('tr');for(const label of ['Dátum','Idő','Prompt','Hívás','Útvonal','Indok']){const th=document.createElement('th');if(label==='Hívás')th.className='calls';escText(th,label);headRow.append(th)}head.append(headRow);table.append(head);const tbody=document.createElement('tbody');for(const [index,item] of group.entries()){const row=document.createElement('tr'),[date,time]=dateAndTime(item.timestamp);appendCell(row,date);appendCell(row,time);appendCell(row,item.prompt_preview||'Nem visszakereshető','prompt-text');appendCell(row,item.api_call_count??index+1,'calls');const route=appendCell(row,'','route');route.append(pill(item.tier||'?',routeKey(item)));appendCell(row,item.reason||'?','reason');tbody.append(row)}table.append(tbody);return table}
function render(){const list=filtered(),body=$('rows');body.replaceChildren();for(const id of ['luna','spark','terra','sol','opus5','qwen'])$(id).textContent=list.filter(e=>e.tier===id).length;$('total').textContent=list.length;
 const gs=groups(list);if(!gs.length){const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=7;td.className='empty';td.textContent='Nincs a szűrésnek megfelelő bejegyzés.';tr.append(td);body.append(tr);return}
...
function executionKind(node){const raw=executionRawCall(node),source=`${node.kind||''} ${raw?.tier||raw?.model||node.model||''}`.toLowerCase();if(node.kind==='LIFECYCLE')return 'lifecycle';if(source.includes('qwen'))return 'qwen';if(source.includes('opus5')||source.includes('claude-opus-5'))return 'opus5';if(source.includes('terra'))return 'terra';if(source.includes('spark'))return 'spark';if(source.includes('luna'))return 'luna';if(source.includes('sol'))return 'sol';return 'worker'}
...
function executionSummary(runCalls){const summary={total:0,luna:0,spark:0,terra:0,sol:0,opus5:0};for(const call of (runCalls||[]).flat()){const tier=executionCallTier(call);summary.total++;if(Object.hasOwn(summary,tier))summary[tier]++}return summary}
...
function appendExecutionNode(tree,node,depth,siblings=[]){const children=node.children||[],hasChildren=children.length>0,open=hasChildren&&taskTreeOpen(node.id,true),kind=executionKind(node),siblingIndex=siblings.indexOf(node),hasPreviousSibling=siblingIndex>0,hasNextSibling=siblingIndex>=0&&siblingIndex<siblings.length-1,row=document.createElement('div');row.className=`execution-tree-row task-tree-${kind} ${node.state==='running'?'running':''} ${hasPreviousSibling?'task-tree-has-prev-sibling':''} ${hasNextSibling?'task-tree-has-next-sibling':''}`;row.style.setProperty('--tree-depth',depth);row.setAttribute('role','treeitem');row.setAttribute('aria-level',String(depth+1));if(hasChildren)row.setAttribute('aria-expanded',String(open));const disclosure=document.createElement(hasChildren?'button':'span');disclosure.className=hasChildren?'execution-tree-toggle':'execution-tree-spacer';disclosure.textContent=hasChildren?(open?'▾':'▸'):'';if(hasChildren){disclosure.type='button';disclosure.title=open?'Belső feladatok bezárása':'Belső feladatok megnyitása';disclosure.addEventListener('click',event=>{event.stopPropagation();setTaskTreeOpen(node.id,!open);render()})}const marker=document.createElement('span');marker.className=`task-tree-marker ${kind}`;marker.setAttribute('aria-hidden','true');const model=document.createElement('span');model.className='execution-tree-model';model.textContent=executionModelLabel(node);const description=document.createElement('span');description.className='execution-tree-description';description.textContent=node.task_description||node.goal||'Nincs megőrzött feladatleírás.';description.title=description.textContent;if(node.external){const badges=[];badges.push('KÜLSŐ');if(node.access_mode==='read_only')badges.push('READ-ONLY');else if(node.access_mode==='requested_read_only')badges.push('KÉRT READ-ONLY');description.textContent=`${node.goal||'Opus review'} · ${badges.join(' · ')} · ${description.textContent}`}const accounting=document.createElement('span');accounting.className='execution-tree-accounting';const ownCalls=executionOwnCount(node),totalCalls=executionTotalCalls(node),metrics=node.metrics||{};accounting.textContent=node.external?`turns ${ownCalls} · input ${metrics.input_tokens??'—'} · output ${metrics.output_tokens??'—'} · cache ${metrics.cache_read_input_tokens??'—'} · cost ${metrics.total_cost_usd??'—'} · duration ${metrics.duration_seconds??'—'}s`:`saját ${ownCalls} · összesített ${totalCalls}`;const routes=document.createElement('span');routes.className='execution-tree-routes';appendRoutePills(routes,executionCalls(node));const state=document.createElement('span'),stateLabels={running:'FUT',success:'SIKER',error:'HIBA',timeout:'TIMEOUT','max-turn':'MAX-TURN',budget:'BUDGET',completed:'KÉSZ'};state.className=`execution-tree-state ${node.state==='running'?'running':''}`;state.textContent=stateLabels[node.state]||String(node.state||'KÉSZ').toUpperCase();row.append(disclosure,marker,model,description,accounting,routes,state);tree.append(row);if(open&&hasChildren){const childrenEl=document.createElement('div');childrenEl.className='execution-tree-children';childrenEl.style.setProperty('--tree-depth',depth);for(const child of children)appendExecutionNode(childrenEl,child,depth+1,children);tree.append(childrenEl)}}
...
function executionState(group,parent){const active=(agentActivity.active_turns||[]).some(turn=>group.some(entry=>String(entry.turn_id||'')===String(turn.turn_id||'')))||(parent?.children||[]).some(child=>child.state==='running');return active?'FUT':'KÉSZ'}
...
render=function(){const allEntries=searchFiltered(),groupedMode=$('grouped').checked,runs=$('runs'),tier=$('tier').value,rootRuns=rootRunRecords(allEntries,tier),limitedRoots=limitRootRuns(rootRuns,$('last').value),displayRoots=[...limitedRoots].sort((left,right)=>groupRunState(left.group)-groupRunState(right.group)),rawList=rawEntriesForRootRuns(limitedRoots,allEntries);runs.replaceChildren();const runData=groupedMode?displayRoots:rawList.map(group=>{const first=group,parent=null,systemCalls=[],scope=executionScope([group],systemCalls,parent,tier);return {group:[group],first,parent,systemCalls,scope}}).filter(run=>!tier||run.scope.calls.length);$('status').textContent=`${limitedRoots.length} root prompt · ${new Date().toLocaleTimeString('hu-HU')}`;const summary=executionSummary(runData.map(run=>run.scope.calls));for(const id of ['luna','spark','terra','sol','opus5'])$(id).textContent=summary[id];$('total').textContent=summary.total;if(!runData.length){const empty=document.createElement('div');empty.className='empty';empty.textContent='Nincs a szűrésnek megfelelő bejegyzés.';runs.append(empty);return}for(const record of runData){const {group,first,parent,scope}=record,key=groupedMode?promptKey(first):`${promptKey(first)}::${first.timestamp}:${first.api_call_count}`,open=isPromptExpanded(key),hasDetails=scope.nodes.length>0,accountingCalls=scope.calls,workerCalls=scope.nodes.filter(node=>node.kind!=='LIFECYCLE').flatMap(executionCalls).length,state=executionState(group,parent),[date,time]=dateAndTime(first.timestamp),run=document.createElement('article');run.className=`router-run ${open?'is-open':'is-closed'} ${state==='FUT'?'running':''}`;const header=document.createElement(hasDetails?'button':'div');if(hasDetails)header.type='button';header.className=`router-run-header ${hasDetails?'':'no-details'}`;if(hasDetails)header.setAttribute('aria-expanded',String(open));if(hasDetails){const toggle=document.createElement('span');toggle.className='router-run-toggle';toggle.textContent=open?'▾':'▸';header.append(toggle)}const dateEl=document.createElement('span');dateEl.className='router-run-date';dateEl.textContent=date;const timeEl=document.createElement('span');timeEl.className='router-run-time';timeEl.textContent=time;const prompt=document.createElement('span');prompt.className='router-run-prompt';prompt.textContent=first.prompt_preview||'Nem visszakereshető';prompt.title=prompt.textContent;const stateEl=document.createElement('span');stateEl.className=`router-run-state ${state==='FUT'?'running':''}`;stateEl.textContent=state==='FUT'?`▶ ÉPPEN FUT · ${(parent?.children||[]).filter(child=>child.state==='running').length||1} AGENT`:'KÉSZ';const total=document.createElement('span');total.className='router-run-total';total.innerHTML='<b>ÖSSZES ROUTING DÖNTÉS</b>';total.append(` ${accountingCalls.length}`);const routes=document.createElement('span');routes.className='router-run-routes';appendRoutePills(routes,accountingCalls);const workers=document.createElement('span');workers.className='router-run-workers';workers.textContent=`${workerCalls} worker-routing`;header.append(dateEl,timeEl,prompt,stateEl,total,routes,workers);if(hasDetails)header.addEventListener('click',()=>{setPromptExpanded(key,!open);render()});run.append(header);if(hasDetails&&open){const panel=document.createElement('section');panel.className='execution-tree-panel';panel.setAttribute('aria-label','Execution tree');panel.append(executionTree(scope.nodes));run.append(panel)}runs.append(run)}};
let refreshInFlight=null;
function refreshDashboard(){if(refreshInFlight)return refreshInFlight;const button=$('refresh'),previousLabel=button.textContent;button.disabled=true;button.textContent='Frissítés…';$('status').className='status refreshing';$('status').textContent=entries.length?'Frissítés… · a korábbi lista látható':'Betöltés…';refreshInFlight=(async()=>{try{const [entriesResponse,agentsResponse]=await Promise.all([fetch(`/api/entries?roots=${$('last').value}`,{cache:'no-store'}),fetch('/api/agents',{cache:'no-store'})]);if(!entriesResponse.ok)throw new Error(`Entries HTTP ${entriesResponse.status}`);if(!agentsResponse.ok)throw new Error(`Agents HTTP ${agentsResponse.status}`);const [payload,activity]=await Promise.all([entriesResponse.json(),agentsResponse.json()]);entries=payload.entries||[];agentActivity=activity;render()}catch(error){$('status').className='status error';$('status').textContent=`Hiba: ${error.message} · a korábbi lista megmaradt`}finally{button.disabled=false;button.textContent=previousLabel;refreshInFlight=null}})();return refreshInFlight}
