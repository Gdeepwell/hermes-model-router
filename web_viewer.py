...
def _save_worker_model(tier, config: dict):
...
async function saveSettings(){
  ...
  try{
    const response=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({callable:currentConfig.callable,workflow:currentConfig.workflow,balance:currentConfig.balance?{enabled:!!currentConfig.balance.enabled,busy_percent:currentConfig.balance.busy_percent,margin_percent:currentConfig.balance.margin_percent}:undefined,default_model:currentConfig.default_model,preferences:currentConfig.preferences||{},hermes_fallback:currentConfig.hermes_fallback||{},worker_model:(currentConfig.worker_model||{}).tier||undefined,usage_limits:Object.fromEntries(Object.entries(currentConfig.accounts||{}).filter(([,i])=>i.guard).map(([a,i])=>[a,{soft_percent:i.soft_percent,hard_percent:i.hard_percent}])),claude_delegation:(currentConfig.accounts||{}).anthropic?{default_tier:currentConfig.accounts.anthropic.delegation.default_tier}:undefined})});
    if(!response.ok)throw new Error(`HTTP ${response.status}`);
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/config":
            ...
            try:
                ...
                # After default_model: the main-agent chain's first entry is the
                # primary, and the fallback save drops it from its own fallbacks.
                ...
                if "worker_model" in data:
                    error = _save_worker_model(data["worker_model"], config)
                    if error:
                        self._send(
                            400,
                            json.dumps({"error": error, "success": False}).encode("utf-8"),
