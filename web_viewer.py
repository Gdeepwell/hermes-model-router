...
.account-cards{display:grid;gap:14px}.account-card{background:linear-gradient(145deg,#151d2c,#0e1420);border:1px solid var(--border);border-radius:14px;padding:15px}.account-card.anthropic{border-color:var(--opus5)}.account-card.openai-codex{border-color:var(--terra)}.account-card.qwen-token{border-color:var(--qwen)}.account-row{display:grid;grid-template-columns:110px 1fr;gap:10px;padding:8px 0;border-top:1px solid #1e2838}.pref-chip.primary select{padding:3px 6px;margin-right:4px}.pref-chip.off{opacity:.6}.panel-heading .settings-status{font-size:12px;font-weight:400;margin-left:10px}.account-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));column-gap:22px;margin-top:8px}.account-grid .account-row{display:flex;flex-direction:column;gap:6px;min-width:0}.account-grid .account-row>span{font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)}.account-grid .account-row.usage{grid-column:1/-1}.account-card .model-switch{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:2px 0;min-height:38px}.account-card .account-effort-model{display:flex;align-items:center;gap:8px;min-height:38px}.account-card .account-effort-model>span{flex:1;min-width:0;font-size:12px}.account-card .account-effort-model select{min-width:0;width:105px;padding:5px 7px}.account-card .account-effort-model .pref-note{margin:0}.account-card .account-row.effort .pref-note{overflow-wrap:anywhere}.account-card .model-switch.disabled>span{color:var(--muted)}.account-card .limits label{display:inline-flex;align-items:center;gap:6px;margin-right:10px}.account-card .limits input{width:64px;padding:5px 7px}.account-card .stepdown{margin-top:6px;font-size:12px;color:var(--muted)}.account-card .usage-line{display:grid;grid-template-columns:60px minmax(0,1fr) 42px 150px;align-items:center;gap:10px;padding:2px 0}.account-card .usage-value{text-align:right}.account-card .usage-reset{font-size:12px;color:var(--muted)}.account-card .usage-age{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:6px;font-size:12px;color:var(--muted)}.account-card .usage-load{margin-left:auto}@media (max-width:760px){.account-grid{grid-template-columns:1fr}.account-card .usage-line{grid-template-columns:50px minmax(0,1fr) 40px}.account-card .usage-reset{grid-column:2/-1}}.usage-bar{position:relative;height:10px;border-radius:99px;background:#1e2838;overflow:hidden}.usage-fill{height:100%}.usage-tick{position:absolute;top:0;width:2px;height:100%;background:#8997ad}.state-badge.open{color:#88e36f}.state-badge.soft{color:#ffb454}.state-badge.closed{color:#ff6b7a}.state-badge.unknown{color:#8997ad}.account-card.stale .usage-bar{opacity:.45}
...
const I18N = {
  en: {
    ...
    'settings.main.off': 'switched off',
    ...
    'settings.claude_effort.heading': 'Claude reasoning effort',
    'settings.claude_effort.sonnet': 'Sonnet',
    'settings.claude_effort.opus': 'Opus',
    'settings.claude_effort.haiku': 'Haiku',
    'settings.claude_effort.haiku_unsupported': 'not adjustable for Haiku',
    'settings.claude_effort.unavailable': 'Not available: {reason}',
  ...
  hu: {
    ...
    'settings.effort.high': 'high',
    'settings.effort.xhigh': 'xhigh',
    'settings.claude_effort.heading': 'Claude gondolkodási erőfeszítés',
    'settings.claude_effort.sonnet': 'Sonnet',
    'settings.claude_effort.opus': 'Opus',
    'settings.claude_effort.haiku': 'Haiku',
    'settings.claude_effort.haiku_unsupported': 'Haikunál nem állítható',
    'settings.claude_effort.unavailable': 'Nem elérhető: {reason}',
...
function renderEffort(tiers){const effort=currentConfig.effort||{},levels=['low','medium','high','xhigh'];
  return tiers.filter(tier=>['luna','spark','terra','sol'].includes(tier)).map(tier=>`<div class="account-effort-model"><span>${t('settings.effort.'+tier)}</span><select data-effort="${tier}">${levels.map(level=>`<option value="${level}"${effort[tier]===level?' selected':''}>${t('settings.effort.'+level)}</option>`).join('')}</select></div>`).join('')}
function renderClaudeReasoningEffort(tiers){const state=currentConfig.claude_reasoning_effort||{available:false,reason:'',levels:{}},levels=state.levels||{},options=['low','medium','high','xhigh'],disabled=state.available?'':' disabled';
  return tiers.map(model=>{const tier={sonnet5:'sonnet',opus5:'opus'}[model];
    if(!tier)return model==='haiku'?`<div class="account-effort-model"><span>${t('settings.claude_effort.haiku')}</span><span class="pref-note">${t('settings.claude_effort.haiku_unsupported')}</span></div>`:'';
    const unavailable=state.available?'':`<span class="pref-note">${(()=>{const message=t('settings.claude_effort.unavailable'),reason=escapeHtml(state.reason||'');return message.includes('{reason}')?message.replace('{reason}',reason):`${message}: ${reason}`})()}</span>`;
    return `<div class="account-effort-model"><span>${t('settings.claude_effort.'+tier)}</span><select data-claude-effort="${tier}"${disabled}>${options.map(level=>`<option value="${level}"${levels[tier]===level?' selected':''}>${t('settings.effort.'+level)}</option>`).join('')}</select>${unavailable}</div>`;
  }).join('')}
function accountCard(account,info){const callable=currentConfig.callable||{},cooldowns=currentConfig.cooldowns||{},load=(currentConfig.load||{})[account]||0,d=info.delegation||{};
  ...
  return `<div class="account-card ${account}${stale?' stale':''}"><div class="account-head"><b>${info.label}</b> <span class="state-badge ${info.state}">${t('account.state.'+info.state)}</span>${liveBadge}</div>`
    ...
    +(effort?`<div class="account-row effort"><span>${t('account.effort')}</span><div>${effort}</div></div>`:'')
...
function fallbackChips(which,chain,modelLabels,rankFrom){
  const options=currentConfig.fallback_options||[],callable=currentConfig.callable||{};
  return chain.map((e,i)=>{
    const known=options.find(o=>o.provider===e.provider&&o.model===e.model);
    const off=known&&known.key&&callable[known.key]===false;
    return `<span class="pref-chip external${off?' off':''}">`
      +`<span class="rank">${i+rankFrom}.</span>${known?fallbackOptionLabel(known,modelLabels):e.provider+'/'+e.model}`
      +(off?`<span class="chain-account-mark">${t('settings.main.off')}</span>`:'')
...
function fallbackPicker(which,chain,modelLabels,exclude){
  const free=(currentConfig.fallback_options||[]).filter(o=>!chain.some(e=>e.provider===o.provider&&e.model===o.model)&&!(exclude&&exclude.provider===o.provider&&exclude.model===o.model));
...
function renderMainChain(modelLabels,models){
  ...
  chains.orchestrator=(chains.orchestrator||[]).filter(e=>!(e.provider===primary.provider&&e.model===primary.model));
...
function renderWorkers(modelLabels){
  ...
  const chain=(currentConfig.hermes_fallback||{}).children||[];
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/usage/refresh":
            ...
            try:
                ...
                router.usage_guard.read(account, config, force=True)
