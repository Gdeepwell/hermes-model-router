# Claude Opus 5.5 coding tier — dispatch plan
...
4. Resolve the canonical effective primary model from Claude Code's JSON `modelUsage`. For an `opus` review, accept when `claude-opus-5-5` is present even if smaller internal/delegated models (for example `claude-haiku-4-5`) also appear; reject only when `claude-opus-5-5` is absent. Append the canonical effective route to the existing JSONL log.
